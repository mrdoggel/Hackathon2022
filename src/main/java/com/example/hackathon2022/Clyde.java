...
public class Clyde extends Ghosts{
    ...
    Image bilde = new Image("file:orangeghost.png");
    
    public Clyde(int bredde, int høyde, Double fart) {
    ...
    public void move() {
        Timeline timeline = new Timeline(new KeyFrame(javafx.util.Duration.seconds(fart), ev -> {
            if (iv.getX()/20 == 15 && iv.getY()/20-0.5 == 14) {
                try {
                    flytt(3);
                    flytt(2);
                    flytt(3);
        ...
        timeline.setDelay(Duration.seconds(45));
