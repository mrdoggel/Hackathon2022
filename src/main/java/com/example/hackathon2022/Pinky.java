...
public class Pinky extends Ghosts{
    ...
    Image bilde = new Image("file:pinkghost.png");
    
    public Pinky(int bredde, int høyde, Double fart) {
    ...
    public void move() {
        Timeline timeline = new Timeline(new KeyFrame(javafx.util.Duration.seconds(fart), ev -> {
            if (iv.getX()/20 == 14 && iv.getY()/20-0.5 == 14) {
                try {
                    flytt(3);
                    flytt(3);
                    flytt(3);
        ...
        timeline.setDelay(Duration.seconds(30));
